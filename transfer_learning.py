...
class Trainer:
    ...
    def validation_epoch(self, epoch):
        ...
        print("Epoch:", epoch, "Current validation loss:", validation_loss, " Accuracy:", validation_acc)
            ...
            for batch_it, (X_batch, Y_batch) in enumerate(self.dataloader_train):
                ...
                if batch_it % self.validation_check == 0:
                    ...
                    if self.should_early_stop():
                        ...
                        return
        torch.save(self.model, "/home/shomeb/h/henrivas/Documents/A2_datasyn/ConvNet-CIFAR/model_res.pt")
...
if __name__ == "__main__":
    ...
    train_loss = np.load('train_v3.npy')
    val_loss = np.load('val_v3.npy')

    plt.figure(figsize=(12, 8))
    plt.title("Comparison")
    plt.plot(trainer.TRAIN_LOSS, label="Resnet18 train loss")
    plt.plot(train_loss, label="Modelv3 train loss")
    plt.plot(trainer.VALIDATION_LOSS, label="Resnet18 val loss")
    plt.plot(val_loss, label="Modelv3 val loss")
    plt.legend()
    plt.savefig(os.path.join("plots", "Comparison_task3.png"))
    plt.show()
